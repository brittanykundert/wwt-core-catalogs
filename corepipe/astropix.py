...
NOTE: AstroPix AVM seems to have confused image parity. Standard JPEGs are
...
from .base import CandidateInput, ImageSource, NotActionableError
...
class AstroPixImageSource(ImageSource):
    ...
    @classmethod
    def deserialize(cls, data: dict) -> "AstroPixImageSource":
        inst = cls()
        inst._json_query_url = data["json_query_url"]
        return inst

    def query_candidates(self) -> Iterable["AstroPixCandidateInput"]:
    ...
    def fetch_candidate(
        ...
        if not source_url:
            raise Exception(
                f"AstroPix invariant violation for {unique_id}: no `resource_url`"
            )
...
class AstroPixCandidateInput(CandidateInput):
    ...
    def save(self, stream: BinaryIO):
        # First check that this input is usable. The NRAO feed contains an item
        # like this, and based on my investigations they are just not usable
        # right now because the server APIs don't work. So: skip any like this.
...
class AstroPixMetadata(object):
    ...
    publisher_id: str
    resource_url: str
    wcs_coordinate_frame: str  # ex: 'ICRS'
    wcs_equinox: str  # ex: 'J2000'
    wcs_projection: str  # ex: 'TAN'
    wcs_reference_dimension: list  # ex: [7416.0, 4320.0]
    wcs_reference_value: list  # ex: [187, 12.3]
    wcs_reference_pixel: (
        list  # ex: [1000.4, 1000.7]; from examples, this seems to be 1-based
    )
    wcs_rotation: float  # ex: -0.07 (deg, presumably)
    wcs_scale: list  # ex: [-6e-7, 6e-7]
    ...
    def as_wcs_headers(self, width: int, height: int) -> dict:
        """
    ...
    def get_credit_url(self) -> str:
        if self.reference_url:
            return self.reference_url

        return "http://www.astropix.org/image/%s/%s" % (
